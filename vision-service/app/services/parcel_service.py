...
from app.cv.coordinate_table_extractor import extract_from_coordinate_table
...
def _run_pipeline(image_bytes: bytes) -> dict:
    try:
        ...
        # Strategy 1: Parse coordinate table (BẢNG LIỆT KÊ TỌA ĐỘ GÓC RANH)
        # More accurate than polygon detection for modern sổ hồng (Nghị định 43/2014)
        table_result = extract_from_coordinate_table(img)
        ...
        # Strategy 2: OpenCV polygon detection (older sổ hồng / diagram-only scans)
        diagram = locate_diagram_region(img)
